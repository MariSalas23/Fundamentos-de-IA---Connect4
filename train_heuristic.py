...
def colocar(tablero: np.ndarray, columna: int, jugador: int) -> np.ndarray | None:
    ...
    se puede colocar ninguna ficha adicional, se devuelve None. '''
...
class HeuristicOpponentMedium:
    ...
    estrategia táctico-heurística: (1) si puede ganar en una jugada, ejecuta
    esa victoria; (2) si el rival puede ganar en una jugada, se bloquea esa
    amenaza; (3) se priorizan las columnas centrales; y (4) se intentan evitar
    jugadas que permitan una victoria inmediata del rival mediante un paso de
    lookahead defensivo. '''
    ...
    def elegir_accion(self, tablero: np.ndarray, jugador: int) -> int:
        ...
        # priorizando aquellas columas donde el rival no consiga ganar en la siguiente jugada.
...
def epsilon_greedy(q_table: dict, estado: str, legales: list[int], epsilon: float) -> int:
    ''' Se implementa una política epsilon-greedy sobre la Q-table, donde con
    probabilidad epsilon se elige una acción aleatoria (exploración) y, en caso
...
def jugar_episodio_qlearning(
    ...
    oponente heurístico. El agente controlado por Q-learning adopta el rol
    ...
    while True:
        ...
        if jugador == agent_player:
            # Se selecciona la acción del agente mediante la política epsilon-greedy.
            ...
            if tablero_siguiente is None:
                # Se castiga con derrota si se intenta jugar en una columna llena (situación que no debería ocurrir).
                recompensa_final = -1
                break
...
def guardar_qstats_completo(q_table: dict, archivo: str) -> None:
    ...
    Q-table tiene la forma q_table[estado][accion] = {"Q": float, "N": int}. '''
...
if __name__ == "__main__":
    ...
    for epi in range(1, episodios_totales + 1):
        # Se aplica un decaimiento lineal sencillo sobre epsilon a lo largo de los episodios.
